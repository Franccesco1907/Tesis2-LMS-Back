...
router.post('/login', (req, res) => {
  ...
  return Employee.findOne()
    ...
    .exec((err, staff) => {
      ...
      // if(!staff) return  res.json({ success: false, message: 'Invalid email...'});
      ...
      //     if(roles){
      //       let objectsRoles = Object.keys(roles);
      //       console.log(objectsRoles)
      //     }
      // })
      console.log(staff);
  ...
  .put(checkJwt, (req, res) => {
    Employee.findOne({ _id: req.decoded.user._id }, (err, staff) => {
      ...
      return staff.save((saveError, updatedStaff) => {
        if (saveError) return res.json({ success: false, message: `Unsuccessful: ${saveError}` });
  ...
  .delete(checkJwt, (req, res) => {
    ...
    Employee.findById(req.decoded.user._id, 'documents', (err, result) => {
      ...
      const index = documents
        .findIndex(({ docName, url }) => (docName === document.docName && url === document.url));
      ...
      return result.save((err, savedR) => {
        if (err) return res.json({ success: false, message: err });
        return res.json({ success: true, message: 'successful', data: savedR.documents });
      });
    });
  });
