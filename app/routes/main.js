...
const { Faculty, Department, ProgrammeModel, CourseModel, StudentModel, Employee, Role } = require('../models');
const { nationality } =  require('../services');
//TO-DO
//work on differentiating applicants from students
router.route('/applicant/register')
    .post((req, res, next) => {
...
router.route('/departments')
    .get((req, res, next) => {
        ...
        if(req.query.faculty) {
            ...
            Faculty.find({}, (err, faculties) => {
                if(err) return  res.json({success: false, message: "Unsucessful " + err});
                
                let result = faculties.filter(faculty => ((query.trim().toLowerCase() === faculty.name.toLowerCase()) || (query.trim() == faculty._id)))[0];
...
router.route('/programmes')
    .get((req, res, next) => {
        ...
        if(req.query.department){
            ProgrammeModel.find({ department: req.query.department }, (err, programmes) => {
                if(err)  return res.json({
                    success: false,
    ...
    .post(checkJwt, adminAuth, (req, res, next) => {
        ...
        let programme = new ProgrammeModel();

        const name = req.body.name;
        const requiredUnits = req.body.requiredUnits;
        const programmeType = req.body.programmeType;
        ...
        ProgrammeModel.find({ name, department })
            .exec((err, result) => {
                if(err) return  res.json({ success: false, message: "Unsuccessful: "+ err.errmsg});
                
                if(result.length) 
                    return  res.json({ success: false, message: `Duplicate entry for ${name}`}); 
   ...
   router.route('/courses')
        ...
        if(Object.keys(query).length !== 0){
            if(query.programmeId) {
                CourseModel.find({ programme: query.programmeId })
                    ...
                    .select(['-session.assignment', '-session.courseMaterial', '-session.courseOutline'])
                    .exec((err, result) => {
                        if(err) return  res.json({ success: false, message: "Unsuccessful! "+ err.errmsg});
                        return res.json({ success: true, message: "Successful", data: result});
                    })
            }
            if(query.departmentId) {/*sort courses out based on department*/}
        }
        else{
            CourseModel.find({})
            .select(['-session.assignment', '-session.courseMaterial', '-session.courseOutline'])
            .exec((err, results) => {
                ...
                return  res.json({success: true, message: "No courses available..."});
            });
        }
    })
    .post(checkJwt, adminAuth, (req, res) => {
        ...
        CourseModel.findOne({ courseName: req.body.courseName, courseCode: req.body.courseCode}, (err, result) => {
            if(err) return res.json({ success: false, message: "Unsuccessful! " + err.errmsg});

            if(result) return  res.json({ success: false, message: "Unsuccessful! Duplicate entry for course"});
    ...
    router.route('/courses/:id')
        .get((req, res, next) => {
            ...
            CourseModel.findOne({ _id: id })
                ...
                .exec((err, course) => {
                    ...
                    if(!course) return  res.json({ success: false, message: "Unsuccessful!...Course not found"});
        ...
        .put( checkJwt, adminAuth, (req, res, next) => {
            ...
            CourseModel.findOne({ _id: id}, (err, result) => {
                ...
                if(!result) return  res.json({ message: "Unsucessful! Course not found" });
                ...
                result.save((err, updatedCourse) => {
                    ...
                    return  res.json({ success: true, message: "Successfully updated course.", data: updatedCourse});
                })
            })
    ...
    router.get('/roles', (req, res, next) => {
        Role.find({}, (err, roles) => {
            ...
            console.log("err", err)
            if(err) return res.json({ success: false, message: "Unsuccessfule"});

            if(!roles) return  res.json({ success: false, message: "No roles yet...Contact admin to add roles"});
